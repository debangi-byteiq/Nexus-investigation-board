...
interface Props {
  ...
  onClearFilters: () => void;
  shifted: boolean;  // when sidebar is open
}
...
const SHAPE_SVG: Record<EntityType, React.ReactNode> = {
  ...
  contact: (
    <svg width="18" height="18" viewBox="0 0 18 18">
      <circle cx="9" cy="9" r="8" fill={ENTITY_COLORS.contact} opacity="0.9" />
    </svg>
  ),
  connection: (
    <svg width="18" height="18" viewBox="0 0 18 18">
      <rect x="1" y="5" width="16" height="8" rx="4" fill={ENTITY_COLORS.connection} opacity="0.9" />
    </svg>
  ),
  annotation: (
    <svg width="18" height="18" viewBox="0 0 18 18">
      <rect x="2" y="1" width="12" height="16" rx="1" fill={ENTITY_COLORS.annotation} opacity="0.9" />
      <polygon points="14,1 18,5 14,5" fill={ENTITY_COLORS.annotation} opacity="0.6" />
    </svg>
  ),
  related: (
    <svg width="18" height="18" viewBox="0 0 18 18">
      <polygon points="9,1 17,9 9,17 1,9" fill={ENTITY_COLORS.related} opacity="0.9" />
    </svg>
  ),
};
...
const LegendPanel: React.FC<Props> = ({ nodes, activeFilters, onToggleFilter, onClearFilters, shifted }) => {
  ...
  const types: EntityType[] = ['case', 'contact', 'connection', 'annotation', 'related'];
